...
class CougCrustDBObject(DatabaseObject):
    ...
    def add_order(self, order: BiteOrder) -> bool:
        return DatabaseObject.add_order(self, order, exclude_fields=["OriginSiteID"],
                                        db_table_name = COUG_CRUST_ORDERS_TABLE_NAME)
    ...
    def add_order_item(self, item: CougCrustOrderItem, order_key: int) -> None:
        ...
        with self.connection.cursor() as cursor:
            try:
                ...
                logging.debug(f"Item successfully added (order_key={order_key}, item={item.to_dict()})")
